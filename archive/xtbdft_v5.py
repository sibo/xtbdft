    ...
    if otherParams == ['skipCrest']:
        ...
        if (os.path.isfile("crest.out")) and ("CREST terminated" in subprocess.check_output(['tail','-1','crest.out']).decode('utf-8')):
         ...
         #run crest on input .xyz file
            ...
            calcID_crest = run_crest(file,chrg,uhf,otherParams=otherParams)
...
def autoTS(file,chrg,uhf,xc,bs,atomNo1,atomNo2,finalScanDistance,otherParams):
    ...
    barrier2=xtbTS("TSguess_forward.xyz",chrg,uhf,atomNo1,atomNo2,finalScanDistance,startScanDistance,"backward")
    if barrier1 < barrier2:
        direction="_forward"
    else:
        direction="_backward"
    copyfile("./TSguess{0}.xyz".format(direction),"./TSguess.xyz")
...
def xtbTS(file,chrg,uhf,atomNo1,atomNo2,startScanDistance,finalScanDistance,direction): 
    ...
    if (os.path.isfile(dir + "/xtbscan.log")):
    ...
    for i in range(1,len(energies)):
        #if the scan was rising, but then experiences a sudden >2kcal/mol drop, then there was a large geometric rearrangement
        if energies[i] > maxEnergy or (energies[i] - energies[i-1] < -2 and rising == True):
        ...
        if energies[i] - energies[i-1] > 0:
            ...
            rising = False
    f=open("../TSguess_{0}.xyz".format(direction),"w+")
